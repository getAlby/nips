...
* **user**: The person using the **client**, and wants to connect their wallet app to their **client**.
...
Description: Fetches the balance of the connected wallet & budget, if it exists.
...
```
where `budget_renewal` can be `daily`, `weekly`, `monthly`, or `yearly` and the integer fields are in msats.
